...
export default function Index({ articles, categories, authors, filters }: Props) {
    ...
    const handleApplyFilters = () => {
        ...
        router.get(articlesRoute.indexAdmin().url, activeFilters, {
            preserveState: true,
            replace: true,
        });
    };
    ...
    const handleDelete = (slug: string) => {
        setIsDeleting(true);
        router.delete(articlesRoute.destroy(slug).url, {
            onSuccess: () => {
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="text-center mb-12">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 text-lg max-w-2xl mx-auto">
                            Kelola, filter, dan cari semua konten artikel Anda dengan mudah.
                        </p>
                    </div>
                            ...
                            <table className="min-w-full text-base">
                                <thead className="bg-gray-100/50 dark:bg-slate-700/50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-left font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider">Tanggal Dibuat</th>
                                    ...
                                    {articles.data.length === 0 ? (
                                        ...
                                        articles.data.map((article) => (
                                            <tr key={article.id} className="hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors duration-200">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">{formatDate(article.created_at)}</td>
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex justify-center items-center gap-2">
                                                        <TooltipProvider delayDuration={100}>
                                                            ...
                                                            <Tooltip><TooltipTrigger asChild><Link href={articlesRoute.edit(article.slug).url}><Button variant="outline" size="icon" className="h-9 w-9 border-blue-300 text-blue-600 hover:bg-blue-50 hover:text-blue-700 dark:border-blue-700 dark:text-blue-400 dark:hover:bg-blue-900/50"><FilePenLine className="h-4 w-4"/></Button></Link></TooltipTrigger><TooltipContent><p>Edit</p></TooltipContent></Tooltip>
                                                            <Tooltip><TooltipTrigger asChild><Button variant="outline" size="icon" onClick={() => setConfirmingDeletionSlug(article.slug)} className="h-9 w-9 border-red-300 text-red-600 hover:bg-red-50 hover:text-red-700 dark:border-red-700 dark:text-red-400 dark:hover:bg-red-900/50"><Trash2 className="h-4 w-4"/></Button></TooltipTrigger><TooltipContent><p>Delete</p></TooltipContent></Tooltip>
                                                        </TooltipProvider>
                                                    </div>
                                                </td>
                                            </tr>
