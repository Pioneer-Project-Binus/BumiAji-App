...
type Article = {
    id: number | string; // ID can be string (ULID) or number
    title: string;
    date: string;
    image: string;
    category: { id: number; name: string } | null; // Category can be null
    slug: string;
    createdAt: string;
    created_at: string;
    status: string;
...
export default function ArticleSection({
    ...
    const handlePageChange = (page: number) => {
        if (page < 1 || page > articles.last_page) return;
        setCurrentPage(page);
    ...
    const handleArticleClick = (article: Article) => {
        router.visit(articlesRoute.showPublic(article.slug).url);
    };

    const handleHighlightClick = () => {
        if (displayHighlight.slug) {
            router.visit(articlesRoute.showPublic(displayHighlight.slug).url);
        }
    };
                                        ...
                                        >
                                            ...
                                            <div className="absolute top-4 left-4 z-10">
                                                <span className="bg-green-700 text-white px-3 py-1 rounded text-xs font-semibold">
                                                    {displayHighlight.category?.name || 'Berita'}
                                                </span>
                                            </div>
                                ...
                                {articles.last_page > 1 && (
                                    <div className="flex justify-center mt-6">
                                        <nav className="flex items-center space-x-2">
                                            <button
                                                className={`w-10 h-10 flex items-center justify-center rounded-md border text-gray-400 bg-gray-100 ${!articles.prev_page_url ? 'cursor-not-allowed' : 'hover:bg-gray-200'
                                                    }`}
                                            ...
                                            <button
                                                className={`w-10 h-10 flex items-center justify-center rounded-md border text-gray-400 bg-gray-100 ${!articles.next_page_url ? 'cursor-not-allowed' : 'hover:bg-gray-200'
                                                    }`}
                                                onClick={() => handlePageChange(articles.current_page + 1)}
