...
export default function IndexPage({ tourismPhotos, destinations, filters }: IndexPageProps) {
    ...
    function performDelete() {
        ...
        destroy(route(tourismPhotosRoutes.destroy(confirmDeleteId).url), {
            preserveState: true,
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto max-w-2xl space-y-6 p-6">
                <div className="flex items-center justify-between">
                    ...
                    <Link href={tourismPhotosRoutes.create().url} className="rounded bg-green-600 px-4 py-2 text-white">
                        + Upload Foto
                    </Link>
                </div>
                            ...
                            tourismPhotos.data.map((photo) => (
                                <tr key={photo.id}>
                                    ...
                                    <td className="space-x-2 border px-4 py-2 text-center">
                                        <Link href={tourismPhotosRoutes.edit(photo.id).url} className="text-blue-600">
                                            Edit
                                        </Link>
