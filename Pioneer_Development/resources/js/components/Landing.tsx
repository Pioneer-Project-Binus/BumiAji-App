...
export default function Landing({ highlight, articles }: LandingProps) {
    ...
    return (
        <div
            className="relative min-h-screen w-full font-sans leading-normal text-gray-800"
                    ...
                    {mappedHighlight && (
                        <SwiperSlide key={mappedHighlight.id} className="h-full">
                            ...
                            <div className="w-full mx-auto flex-1 bg-gray-300 flex items-center text-gray-500 relative">
                                ...
                                <Link href={`/artikel/${mappedHighlight.id}`} className="absolute object-cover w-full h-full">
                            ...
                            <div className="bg-[#25603B] shadow-md p-8 text-start">
                                <Link href={`/artikel/${mappedHighlight.id}`} className="font-bold text-xl mb-2 text-white hover:underline block">{mappedHighlight.title}</Link>
                ...
                <div className="flex flex-col h-[550px]">
                    <div className="flex-grow overflow-y-auto space-y-6" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                        ...
                        <div key={item.id} className="bg-white rounded-lg shadow-[5px_5px_10px_rgba(0,0,0,0.3)] flex flex-col lg:flex-row w-full">
                            ...
                            <Link href={`/articles/${item.id}`} className="font-bold text-lg mb-1 lg:mb-2 hover:underline block">{item.title}</Link>
                    ...
                    <div className="flex justify-end pt-4">
                        <Link href="/artikel" className="font-bold text-xl text-black hover:underline">Berita Lainya...</Link>
                    </div>
                </div>
            </div>
            </section>
